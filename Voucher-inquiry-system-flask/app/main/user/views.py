...
from .forms import EditProfileForm, AvatarEditForm, AvatarUploadForm,RemovePasswordForm,ChangePermissionForm
...
#撤销密码
@permission_required(Permission.ADMINISTER)
@user.route('/<int:user_id>/remove_password/', methods=['GET', 'POST'])
@login_required
def remove_password(user_id):
    if current_user.id == user_id or current_user.can(Permission.UPDATE_OTHERS_INFORMATION):
        the_user = User.query.get_or_404(user_id)
        form = RemovePasswordForm()
        if form.validate_on_submit():
            the_user.password = form.new_password.data
            db.session.add(current_user)
            db.session.commit()
            flash(u'密码更新成功!', 'info')
            return redirect(url_for('user.detail', user_id=user_id))
        return render_template('user_edit.html', form=form, user=the_user, title=u"修改密码")
    else:
        abort(403)
...
#改变权限
@permission_required(Permission.ADMINISTER)
@user.route('/<int:user_id>/change_permission/', methods=['GET', 'POST'])
@login_required
def change_permission(user_id):
    if current_user.id == user_id or current_user.can(Permission.UPDATE_OTHERS_INFORMATION):
        ...
        if form.validate_on_submit():
            the_user.role_id = form.Permission.data
            db.session.add(current_user)
            db.session.commit()
            flash(u'权限更新成功!', 'info')
            return redirect(url_for('user.detail', user_id=user_id))
        return render_template('user_edit.html', form=form, user=the_user, title=u"更新权限")
    else:
        abort(403)
